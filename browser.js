...
if (process.argv.length < 8) {
  ...
  console.log('');
  console.log(colored(colors.COLOR_CYAN, "                                        13 August, 2024", true));
  console.log('                          -------------------------------------------');
  console.log(`
    ${colored(colors.COLOR_BRIGHT_PURPLE, 'PUPPETEER v1.2')} | Latest update For Cloudflare Captcha Bypass , Speed Captcha Solving With More Options Added Can Help The Solution Become More Optimized
 ...
 if (enabled('fin')) {
        await page.evaluateOnNewDocument(() => {
          ...
          [
            ...
            { prop: 'hardwareConcurrency', value: 4 },
            { prop: 'deviceMemory', value: 0.25 },
            { prop: 'vendor', value: 'Google Inc.' },
            { prop: 'webdriver', value: false },
          ].forEach(({ prop, value }) => setNavigatorProperty(prop, value));

          // Thi?t l?p các thu?c tính cho d?i tu?ng screen
