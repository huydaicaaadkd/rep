    ...
    clients.forEach(client => {
        const intervalId = setInterval(async () => {
            ...
            threadvalue += 1
                ...
                let streamId =1;
                let streamIdReset = 1;
                const flags = 0x1 | 0x4 | 0x20;  // Bitwise OR combination of flags
                const encodedFrame = encodeFrame(streamId, 1, packed, flags);  // Encode the frame
                const frame = Buffer.concat([encodedFrame]);  // Concatenate into a single buffer
